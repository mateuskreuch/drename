...
**`drename`** is a Python CLI tool for recursively renaming files, directories, and text contents inside files by replacing occurrences of a target string (`OLD`) with a new string (`NEW`), while **preserving case style**.
...
`drename` will preserve the case style of the original text. You indicate word bounds by using a slash (/).
